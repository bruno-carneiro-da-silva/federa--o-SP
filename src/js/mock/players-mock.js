const names = [
  "BBOY ZEUS",
  "BGIRL VILOETTA",
  "BGIRL RÁ",
  "BGIRL LUA",
  "BGIRL CARLA",
  "BGIRL DANDARA",
  "BGIRL DÉBB KILLA",
  "BBOY BABY",
  "BBOY PIKIRA",
  "BBOY SPIDER",
  "BBOY SAGARA",
  "BGIRL FURAÇÃO",
  "BBOY BIEL",
  "BBOY JASON",
  "BGIRL MATEA",
  "BBOY MONGE",
  "BBOY RUI",
  "BGIRL JEOVANA",
  "BBOY FLASH",
  "BBOY CHOSEN",
  "BBOY GUI",
  "BBOY LULLA",
  "BBOY KAEL",
  "BGIRL NIX",
  "BGIRL NICK",
  "BBOY ICE",
  "BBOY RICHARD",
  "BBOY BIEL MAC FLY",
  "BBOY POLAR",
  "DJ KAMARAUM",
  "BBOY ZYM",
  "BBOY EAGLE",
  "BGIRL ANGEL DO BRASIL",
  "BBOY AYLSON",
  "BBOY JHEFF",
  "BGIRL KEKÉ",
];

const players = [
  {
    id: 1,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202423809",
    position: "3",
    meta: "AT-01 / Filiado",
    name: names[0],

points: 100,  },
  {
    id: 2,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202427152",
    meta: "AT-02 / Filiado",
    name: names[1],

points: 100,  },
  {
    id: 3,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202428826",
    meta: "AT-03 / Filiado",
    name: names[2],

points: 100,  },
  {
    id: 4,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Caetano do Sul",
    category: "Adulto",
    NI: "202421811",
    meta: "AT-04 / Filiado",
    name: names[3],

points: 100,  },
  {
    id: 5,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202425106",
    meta: "AT-05 / Filiado",
    name: names[4],

points: 100,  },
  {
    id: 6,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Mogi das Cruzes",
    category: "Adulto",
    NI: "202429827",
    meta: "AT-06 / Filiado",
    name: names[5],

points: 100,  },
  {
    id: 7,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202425861",
    meta: "AT-07 / Filiado",
    name: names[6],

points: 100,  },
  {
    id: 8,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo/França",
    category: "Adulto",
    NI: "202424836",
    meta: "AT-08 / Filiado",
    name: names[7],

points: 100,  },
  {
    id: 9,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202422669",
    meta: "AT-09 / Filiado",
    name: names[8],

points: 100,  },
  {
    id: 10,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Kids",
    NI: "202424800",
    meta: "AT-10 / Filiado",
    name: names[9],

points: 100,  },
  {
    id: 11,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Caetano do Sul",
    category: "Adulto",
    NI: "202420802",
    meta: "AT-11 / Filiado",
    name: names[10],
    points: 100,
  },
  {
    id: 12,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202421864",
    meta: "AT-12 / Filiado",
    name: names[11],
    points: 100,
  },
  {
    id: 13,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202424800",
    meta: "AT-13 / Filiado",
    name: names[12],
    points: 100,
  },
  {
    id: 14,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202421359",
    meta: "AT-14 / Filiado",
    name: names[13],
    points: 100,
  },
  {
    id: 15,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202420870",
    meta: "AT-15 / Filiado",
    name: names[14],
    points: 100,
  },
  {
    id: 16,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Hortolândia",
    category: "Adulto",
    NI: "202424817",
    meta: "AT-16 / Filiado",
    name: names[15],
    points: 100,
  },
  {
    id: 17,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São José do Rio Pardo",
    category: "Adulto",
    NI: "202420801",
    meta: "AT-17 / Filiado",
    name: names[16],
    points: 100,
  },
  {
    id: 18,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202429681",
    meta: "AT-18 / Filiado",
    name: names[17],
    points: 100,
  },
  {
    id: 19,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202422489",
    meta: "AT-19 / Filiado",
    name: names[18],
    points: 100,
  },
  {
    id: 20,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Youth",
    NI: "202425840",
    meta: "AT-20 / Filiado",
    name: names[19],
    points: 100,
  },
  {
    id: 21,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Kids",
    NI: "202426876",
    meta: "AT-21 / Filiado",
    name: names[20],
    points: 100,
  },
  {
    id: 22,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Campinas",
    category: "Adulto",
    NI: "202424851",
    meta: "AT-22 / Filiado",
    name: names[21],
    points: 100,
  },
  {
    id: 23,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Bernardo do Campo",
    category: "Adulto",
    NI: "202421318",
    meta: "AT-23 / Filiado",
    name: names[22],
    points: 100,
  },
  {
    id: 24,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202423875",
    meta: "AT-24 / Filiado",
    name: names[23],
    points: 100,
  },
  {
    id: 25,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Kids",
    NI: "202429889",
    meta: "AT-25 / Filiado",
    name: names[24],
    points: 100,
  },
  {
    id: 26,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Diadema",
    category: "Kids",
    NI: "202429803",
    meta: "AT-26 / Filiado",
    name: names[25],
    points: 100,
  },
  {
    id: 27,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202422810",
    meta: "AT-27 / Filiado",
    name: names[26],
    points: 100,
  },
  {
    id: 28,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Diadema",
    category: "Adulto",
    NI: "202429818",
    meta: "AT-28 / Filiado",
    name: names[27],
    points: 100,
  },
  {
    id: 29,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São José do Rio Pardo",
    category: "Adulto",
    NI: "202421873",
    meta: "AT-29 / Filiado",
    name: names[28],
    points: 100,
  },
  {
    id: 30,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Rio Claro",
    category: "Adulto",
    NI: "202529873",
    meta: "AT-30 / Filiado",
    name: names[29],
    points: 100,
  },
  {
    id: 31,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Porto Ferreira",
    category: "Adulto",
    NI: "202525830",
    meta: "AT-31 / Filiado",
    name: names[30],
    points: 100,
  },
  {
    id: 32,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Adulto",
    NI: "202524821",
    meta: "AT-32 / Filiado",
    name: names[31],
    points: 100,
  },
  {
    id: 33,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "São Paulo",
    category: "Youth/Kids",
    NI: "202521803",
    meta: "AT-33 / Filiado",
    name: names[32],
    points: 100,
  },
  {
    id: 34,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Mongaguá",
    category: "Adulto",
    NI: "202523882",
    meta: "AT-34 / Filiado",
    name: names[33],
    points: 100,
  },
  {
    id: 35,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Itanhaém",
    category: "Adulto",
    NI: "202526835",
    meta: "AT-35 / Filiado",
    name: names[34],
    points: 100,
  },
  {
    id: 36,
    imgSrc: "images/logo-original.png",
    videoUrl: "https://vimeo.com/139714818",
    city: "Itanhaém",
    category: "Adulto",
    NI: "202522851",
    meta: "AT-36 / Filiado",
    name: names[35],
    points: 100,
  },
];

const playersInsights = [
  {
    id: 1,
    imgSrc: "images/players/marininha.jpg",
    name: "BGIL MARININHA",
    role: "ADULTO",
    description:
      "As notas dos atletas mostradas a seguir são baseadas e possuem validade até o próximo campeonato da federação. Desta forma, os dados serão atualizados a cada campeonato, e elas são baseadas nos filtros realizados no evento.",
    status: {
      gamesPlayed: 22,
      minutesPlayed: 1256,
      goals: 30,
      assists: 41,
      jerseyNumber: 10,
    },
    percentages: {
      possing: 62,
      tackles: 39,
      dribbling: 77,
    },
  },
  {
    id: 2,
    imgSrc: "images/players/debbkilla-1.jpg",
    name: "BGIRL DEBB KILA",
    role: "ADULTO",
    description:
      "As notas dos atletas mostradas a seguir são baseadas e possuem validade até o próximo campeonato da federação. Desta forma, os dados serão atualizados a cada campeonato, e elas são baseadas nos filtros realizados no evento.",
    status: {
      gamesPlayed: 23,
      minutesPlayed: 987,
      goals: 26,
      assists: 55,
      jerseyNumber: 8,
    },
    percentages: {
      possing: 56,
      tackles: 34,
      dribbling: 68,
    },
  },
  {
    id: 3,
    imgSrc: "images/players/nix.jpeg",
    name: "BGIRL NIX",
    role: "ADULTO",
    description:
      "As notas dos atletas mostradas a seguir são baseadas e possuem validade até o próximo campeonato da federação. Desta forma, os dados serão atualizados a cada campeonato, e elas são baseadas nos filtros realizados no evento.",
    status: {
      gamesPlayed: 23,
      minutesPlayed: 987,
      goals: 26,
      assists: 55,
      jerseyNumber: 8,
    },
    percentages: {
      possing: 56,
      tackles: 34,
      dribbling: 68,
    },
  },
  {
    id: 3,
    imgSrc: "images/players/richard-1.jpg",
    name: "BBOY RICHARD",
    role: "ADULTO",
    description:
      "As notas dos atletas mostradas a seguir são baseadas e possuem validade até o próximo campeonato da federação. Desta forma, os dados serão atualizados a cada campeonato, e elas são baseadas nos filtros realizados no evento.",
    status: {
      gamesPlayed: 23,
      minutesPlayed: 987,
      goals: 26,
      assists: 55,
      jerseyNumber: 8,
    },
    percentages: {
      possing: 56,
      tackles: 34,
      dribbling: 68,
    },
  },
  {
    id: 4,
    imgSrc: "images/players/zym.jpg",
    name: "BBOY ZYM",
    role: "ADULTO",
    description:
      "As notas dos atletas mostradas a seguir são baseadas e possuem validade até o próximo campeonato da federação. Desta forma, os dados serão atualizados a cada campeonato, e elas são baseadas nos filtros realizados no evento.",
    status: {
      gamesPlayed: 23,
      minutesPlayed: 987,
      goals: 26,
      assists: 55,
      jerseyNumber: 8,
    },
    percentages: {
      possing: 56,
      tackles: 34,
      dribbling: 68,
    },
  },
  {
    id: 6,
    imgSrc: "images/players/lulla.jpg",
    name: "BBOY LULLA",
    role: "ADULTO",
    description:
      "As notas dos atletas mostradas a seguir são baseadas e possuem validade até o próximo campeonato da federação. Desta forma, os dados serão atualizados a cada campeonato, e elas são baseadas nos filtros realizados no evento.",
    status: {
      gamesPlayed: 23,
      minutesPlayed: 987,
      goals: 26,
      assists: 55,
      jerseyNumber: 8,
    },
    percentages: {
      possing: 56,
      tackles: 34,
      dribbling: 68,
    },
  },
];

export { players, playersInsights };
